...
test('upload cleanup preserves RecordTypeId while removing system and unknown fields', () => {
	const knownFields = new Set(['Name']);
	assert.equal(uploadModal.shouldStripUploadField('RecordTypeId', knownFields), false);
	assert.equal(uploadModal.shouldStripUploadField('Name', knownFields), false);
	assert.equal(uploadModal.shouldStripUploadField('Id', knownFields), true);
	assert.equal(uploadModal.shouldStripUploadField('Source_Only__c', knownFields), true);
});

test('record access excludes only the operation Salesforce will reject', () => {
	const readOnlyUpdate = {
		loadedFromId: '001000000000001AAA',
		_recordAccess: { checked: true, hasEditAccess: false, hasDeleteAccess: false },
	};
	assert.equal(uploadModal.recordAccessWriteReason(readOnlyUpdate, true), 'no-edit-access');
	assert.equal(uploadModal.recordAccessWriteReason(readOnlyUpdate, false), null);
	readOnlyUpdate.pendingDelete = true;
	assert.equal(uploadModal.recordAccessWriteReason(readOnlyUpdate, true), 'no-delete-access');
	readOnlyUpdate._recordAccess.hasDeleteAccess = true;
	assert.equal(uploadModal.recordAccessWriteReason(readOnlyUpdate, true), null);
});
