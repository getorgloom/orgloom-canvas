...
describe('accountsDb.listForAdminView', () => {
	...
	test('filters accounts by a case-insensitive email substring', async () => {
		const { accounts } = await import('../src/database/index.js');
		const match = (await accounts.upsertByEmail({ email: 'Casey.Search@Example.com' })).account;
		await accounts.upsertByEmail({ email: 'someone-else@example.com' });

		const list = await accounts.listForAdminView({ emailQuery: 'SEARCH@EXAMPLE' });
	...
	test('email filtering still respects deleted-account visibility', async () => {
		const { accounts } = await import('../src/database/index.js');
		const deleted = (await accounts.upsertByEmail({ email: 'deleted-search@example.com' })).account;
		await accounts.softDelete(deleted.id);

		assert.equal((await accounts.listForAdminView({ emailQuery: 'deleted-search' })).length, 0);
		assert.equal(
			(await accounts.listForAdminView({ emailQuery: 'deleted-search', includeDeleted: true })).length,
			1,
		);
	});
});
