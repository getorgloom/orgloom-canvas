...
test('a launch-link result is not masked by the routine session-restore notice', () => {
	...
	assert.match(source, /_restored[\s\S]*!_hasCanvasLaunchRequest[\s\S]*Restored your unsaved canvas from this tab/);
	assert.match(
		source,
...
test('an inaccessible share link shows the standard canvas-unavailable dialog', () => {
	const start = source.indexOf('(function autoOpenSharedCanvas()');
	const end = source.indexOf('\n\t})();', start);
	const handler = source.slice(start, end);
	assert.match(handler, /showCanvasLaunchError\(/);
	assert.match(handler, /data && \(data\.message \|\| data\.error\)/);
	assert.match(handler, /finally \{\s*clearCanvasLaunchParam\('share'\)/);
	assert.ok(
		handler.indexOf("document.body.classList.add('canvas-recipient-mode')") > handler.indexOf('if (!r.ok)'),
		'recipient mode should only be enabled after the canvas opens successfully',
	);
});
