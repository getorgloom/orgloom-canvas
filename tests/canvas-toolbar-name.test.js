...
test('toolbar blocks Upload to Salesforce before opening review when permission is missing', () => {
	assert.match(toolbarSource, /hasCapability\('upload-records'\)/);
	assert.match(toolbarSource, /const uploadCapabilityReady = isCapabilityReady\(\)/);
	assert.match(toolbarSource, /Ask a workspace admin to grant you the Upload to Salesforce permission/);
	assert.match(toolbarSource, /upload-btn upload-btn--locked" disabled/);
	assert.match(appSource, /hasCapability:[\s\S]*_hasCap\.apply/);
	assert.match(appSource, /isCapabilityReady:[\s\S]*return _capsLoaded/);
});

test('toolbar reserves pending capability controls without flashing denial locks', () => {
	assert.match(
		toolbarSource,
		/!uploadCapabilityReady[\s\S]*class="upload-btn" disabled aria-disabled="true" aria-busy="true"/,
	);
	assert.match(toolbarSource, /const savePending = !saveCapabilityReady/);
	assert.match(toolbarSource, /shareState = 'checking-access'[\s\S]*shareDisabled = true/);
	assert.match(menuSource, /const aiPending = !aiAccess\.ready/);
	assert.doesNotMatch(toolbarSource, /!uploadCapabilityReady[\s\S]{0,180}upload-btn--locked/);
});
...
test('schema controls wrap inside the schema pane without covering canvas save state', () => {
	assert.match(cssSource, /\.graph-split\s*\{[\s\S]*--schema-pane-width: clamp\(380px, 35%, 660px\)/);
	assert.match(
		cssSource,
		/\.schema-controls-overlay\s*\{[\s\S]*justify-content: flex-start[\s\S]*width: calc\(var\(--schema-pane-width,[\s\S]*\) - 16px\)/,
	);
	assert.match(cssSource, /\.graph-filter\s*\{\s*display: contents;\s*\}/);
	assert.match(cssSource, /\.graph-filter input\s*\{[\s\S]*max-width: 220px/);
	assert.match(appSource, /id="graph-filter-wrap"[\s\S]*id="graph-rel-filter"[\s\S]*id="schema-system-fields-chip"/);
	assert.match(appSource, /const MIN_CANVAS = 280/);
	assert.match(appSource, /split\.style\.setProperty\('--schema-pane-width', w \+ 'px'\)/);
});
