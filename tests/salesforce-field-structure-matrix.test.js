...
describe('Salesforce field-structure matrix', () => {
	...
	test('external-key references are recognized and uploaded as writable scalar keys', () => {
		...
		assert.equal(isExternalKeyReference(externalLookup), true);
		assert.equal(isWritableForOperation(externalLookup, 'create'), true);
		assert.equal(isWritableForOperation(externalLookup, 'update'), true);
		assert.deepEqual(
			stripUnwritableFields(
				{ Name: 'Updated', ExternalParent__c: 'ORDER-1025' },
				{
					fields: [{ name: 'Name', type: 'string', createable: true, updateable: true }, externalLookup],
				},
				true,
			),
			{ Name: 'Updated', ExternalParent__c: 'ORDER-1025' },
		);
		assert.deepEqual(stripUnwritableFields({ ExternalParent__c: null }, { fields: [externalLookup] }, true), {
			ExternalParent__c: null,
		});
	});
	...
	test('projected describes preserve record-type, dependency, indirect-lookup, and master-detail metadata', async () => {
		...
		assert.equal(byName.get('Description__c').htmlFormatted, true);
		assert.equal(byName.get('Description__c').compoundFieldName, 'Description_Group__c');
		assert.equal(byName.get('Calculated_Time__c').type, 'time');
		assert.equal(byName.get('Calculated_Time__c').calculated, true);
	});
});
