...
test('saved canvases retain encrypted field intent without retaining encrypted values', () => {
	const payload = {
		loadedRecords: [
			{
				objectName: 'Account',
				values: { Name: 'Acme', Secret__c: 'plain' },
				loadedValues: { Name: 'Old', Secret__c: 'prior' },
				changes: { Name: 'Acme', Secret__c: 'plain' },
				slot: { slotId: 'loaded-fields', kind: 'fields', fields: ['Name', 'Secret__c'] },
			},
		],
		drafts: [
			{
				objectName: 'Account',
				values: { Name: 'Draft', Secret__c: 'draft secret' },
				slot: { slotId: 'draft-fields', kind: 'fields', fields: ['Name', 'Secret__c'] },
			},
		],
	};
	const access = new Map([['Account', { encryptedFields: new Set(['Secret__c']) }]]);
	const clean = stripEncryptedCanvasValues(payload, access);

	assert.deepEqual(clean.loadedRecords[0].values, { Name: 'Acme' });
	assert.deepEqual(clean.loadedRecords[0].loadedValues, { Name: 'Old' });
	assert.deepEqual(clean.loadedRecords[0].changes, { Name: 'Acme' });
	assert.deepEqual(clean.loadedRecords[0].encryptedFieldIntents, ['Secret__c']);
	assert.deepEqual(clean.loadedRecords[0].slot.fields, ['Name']);
	assert.deepEqual(clean.drafts[0].values, { Name: 'Draft' });
	assert.deepEqual(clean.drafts[0].encryptedFieldIntents, ['Secret__c']);
	assert.deepEqual(clean.drafts[0].slot.fields, ['Name']);
	assert.equal(payload.loadedRecords[0].values.Secret__c, 'plain');
});
...
describe('shared canvas projection', () => {
	...
	test('does not project encrypted field values or requests from an owner into a shared canvas', () => {
				...
				loadedRecords: [
					{
						objectName: 'Account',
						loadedFromId: '001000000000001AAA',
						values: { Name: 'Visible', Encrypted__c: 'decrypted owner value' },
						changes: { Encrypted__c: 'staged decrypted value' },
						slot: { slotId: 'fields', kind: 'fields', fields: ['Name', 'Encrypted__c'] },
					},
				],
				drafts: [
					{
						tempId: 'draft-1',
						objectName: 'Account',
						values: { Name: 'Draft', Encrypted__c: 'draft secret' },
					},
				],
				schema: { objects: [{ name: 'Account', draftFields: [encryptedField] }] },
				associations: [],
			},
			new Map([
				[
					'Account',
					{
						...
						queryable: true,
						label: 'Account',
						readableFields: new Set(['Name', 'Encrypted__c']),
						fields: new Map([
							['Name', { name: 'Name', label: 'Name', type: 'string' }],
							['Encrypted__c', encryptedField],
						]),
					},
				],
			]),
		...
		assert.deepEqual(projected.loadedRecords[0].values, { Name: 'Visible' });
		assert.equal(projected.loadedRecords[0].changes, undefined);
		assert.deepEqual(projected.loadedRecords[0].slot.fields, ['Name']);
		assert.equal(projected.loadedRecords[0].slot.unavailableFieldCount, 1);
		assert.deepEqual(projected.drafts[0].values, { Name: 'Draft' });
		assert.equal(JSON.stringify(projected).includes('decrypted owner value'), false);
		assert.equal(JSON.stringify(projected).includes('staged decrypted value'), false);
		assert.equal(JSON.stringify(projected).includes('draft secret'), false);
		assert.equal(JSON.stringify(projected).includes('Encrypted__c'), false);
	});
