...
describe('classifyValueDrift', () => {
	test('includes the object label and current Salesforce record name', async () => {
		const conn = makeQueryConn({
			'001abc': { Id: '001abc', Name: 'Acme Holdings', Industry: 'Banking' },
		});
		...
		assert.equal(result.records[0].objectLabel, 'Customer Account');
		assert.equal(result.records[0].recordName, 'Acme Holdings');
	});
	...
	test('loose equality covers JSON round-trip coercions (5 vs "5")', async () => {
		...
		const row = makeUpdateRow({
			...
			uploadedValues: { NumberField: '5' }, // String due to JSON round-trip
		});
		const result = await classifyValueDrift({ conn, batch: { insertedIds: [row] } });
		const rec = result.records[0];
		assert.equal(rec.clean.length, 1);
...
describe('executeRecall - value-revert path', () => {
	...
	test('revertSelections fires one PATCH per record with the selected fields', async () => {
		...
		assert.equal(result.revertedFieldCount, 2);
		assert.equal(result.revertFailedCount, 0);
	...
	test('execution rechecks drift and never overwrites a value changed after review', async () => {
		...
		assert.equal(result.status, 'recall_failed');
	...
	test('explicit changed-field restore overwrites the exact value reviewed by the user', async () => {
		const conn = makeRecallConn({
			executionRecords: [{ Id: '001abc', Industry: 'Third-party edit' }],
		});
		const batch = {
		...
		const result = await executeRecall({
			...
			forceRevertSelections: [
				{
					sfId: '001abc',
					fields: [{ fieldName: 'Industry', expectedCurrent: 'Third-party edit' }],
				},
			],
		});
		assert.equal(conn.calls.updates.length, 1);
		assert.equal(conn.calls.updates[0].patch.Industry, 'Tech');
		assert.equal(result.revertedFieldCount, 1);
		assert.equal(result.forcedRevertedFieldCount, 1);
		assert.equal(result.revertDriftSkippedCount, 0);
		assert.equal(result.status, 'recalled');
	});

	test('changed-field restore preserves a value that changed again after review', async () => {
		const conn = makeRecallConn({
			executionRecords: [{ Id: '001abc', Industry: 'Changed again' }],
		});
		const batch = {
			insertedIds: [
				makeUpdateRow({
		...
		const result = await executeRecall({
			...
			forceRevertSelections: [
				{
					sfId: '001abc',
					fields: [{ fieldName: 'Industry', expectedCurrent: 'Value shown in review' }],
				},
			],
		});
		assert.equal(conn.calls.updates.length, 0);
		assert.equal(result.forcedRevertedFieldCount, 0);
		assert.equal(result.revertDriftSkippedCount, 1);
		assert.equal(result.status, 'recall_failed');
	});
	...
	test('SF PATCH failure surfaces in revertResults; revertFailedCount ticks', async () => {
		...
		assert.equal(result.revertFailedFieldCount, 1);
		assert.equal(result.status, 'recall_failed');
