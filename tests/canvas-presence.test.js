...
describe('canvas presence security and ordering', () => {
	...
	test('canonicalizes a stale saved-draft layout reference before relaying it to an editor', () => {
		const canvasId = '069000000000097AAA';
		const payload = {
			schema: { objects: [{ name: 'Account', label: 'Account' }] },
			loadedRecords: [],
			drafts: [
				{
					tempId: 'saved-editor-draft',
					canvasRecordId: 'editor-created-card',
					objectName: 'Account',
					values: {},
					x: 10,
					y: 20,
		...
		const ownerConnection = subscribe({
			canvasId,
			workspaceId: 'w',
			accountId: 'owner-saved-draft-layout',
			role: 'owner',
			canEdit: true,
			sseRes: owner,
		});
		subscribe({
			canvasId,
			workspaceId: 'w',
			accountId: 'editor-saved-draft-layout',
			role: 'editor',
			canEdit: true,
			visibility: {
				loadedRecords: {},
				drafts: { 'saved-editor-draft': { visible: true, readableFields: ['Name'] } },
		...
		assert.equal(
			updateLayout({
				...
				sequence: 1,
				requestingAccountId: 'owner-saved-draft-layout',
			}),
			true,
		);

		const layout = events(editor).find((event) => event.type === 'record-layout');
		assert.deepEqual(layout.positions, [
			{
				refKind: 'draft',
				ref: 'saved-editor-draft',
				collabRef: 'editor-created-card',
				x: 450,
				y: 550,
			},
		...
		assert.deepEqual(
			{ x: liveSnapshot({ canvasId }).payload.drafts[0].x, y: liveSnapshot({ canvasId }).payload.drafts[0].y },
			{ x: 450, y: 550 },
		);
		owner.fire('close');
		editor.fire('close');
	...
	test('projects loaded-record and relationship events through recipient Salesforce visibility', () => {
		...
		subscribe({
			...
			visibility: {
				loadedRecords: {
					'003000000000001': {
						objectName: 'Contact',
						readableFields: ['Name', 'AccountId', 'Encrypted__c'],
						encryptedFields: ['Encrypted__c'],
					},
					'001000000000001': {
						objectName: 'Account',
						readableFields: ['Name'],
		...
		assert.equal(
			updateLoadedRecord({
				...
				fields: { Name: 'Visible', Secret__c: 'hidden', Encrypted__c: 'decrypted owner value' },
				sequence: 1,
				requestingAccountId: 'owner',
			}),
			true,
		);
		const loadedEvent = events(viewer).find((event) => event.type === 'loaded-record');
		assert.deepEqual(loadedEvent.fields, { Name: 'Visible' });
		assert.deepEqual(loadedEvent.redactedFields, ['Encrypted__c']);
	...
	test('does not relay an editor decrypted field value to a canvas owner', () => {
		const owner = response();
		const editor = response();
		const canvasId = '069000000000098AAA';
		subscribe({
			...
			visibility: {
				loadedRecords: {
					'001000000000001': {
						objectName: 'Account',
						readableFields: ['Name', 'Encrypted__c'],
						encryptedFields: ['Encrypted__c'],
					},
				},
				drafts: {},
		...
		assert.equal(
			updateLoadedRecord({
				...
				objectName: 'Account',
				fields: { Name: 'Visible', Encrypted__c: 'editor decrypted value' },
				baseline: { Name: 'Visible', Encrypted__c: 'editor decrypted value' },
				sequence: 1,
				requestingAccountId: 'editor',
			}),
			true,
		);
		const event = events(owner).find((candidate) => candidate.type === 'loaded-record');
		assert.deepEqual(event.fields, { Name: 'Visible' });
		assert.deepEqual(event.baseline, { Name: 'Visible' });
		assert.deepEqual(event.redactedFields, ['Encrypted__c']);
	});
