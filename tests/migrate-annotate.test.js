...
function field(name, opts) {
	...
	return {
		...
		picklistValuesByRecordType: opts.picklistValuesByRecordType || {},
		controllerName: opts.controllerName || null,
		controllerValues: opts.controllerValues || null,
		controllerValuesByRecordType: opts.controllerValuesByRecordType || {},
		restrictedPicklist: opts.restrictedPicklist,
	};
}
function pv(value) {
...
describe('computeMigrationStatus', () => {
	...
	test('record-type picklist values override the generic destination options', () => {
		const d = {
			defaultRecordTypeId: '012DEFAULT',
			recordTypes: [{ developerName: 'Business', id: '012BUSINESS' }],
			fields: [
				field('Stage', {
					type: 'picklist',
					restrictedPicklist: true,
					picklistValues: [pv('GenericOnly')],
					picklistValuesByRecordType: { '012BUSINESS': [pv('BusinessOnly')] },
				}),
			],
		};
		const valid = {
			values: { Stage: 'BusinessOnly' },
			_sourceRecordTypeDeveloperName: 'Business',
		};
		const invalid = {
			values: { Stage: 'GenericOnly' },
			_sourceRecordTypeDeveloperName: 'Business',
		};

		assert.equal(annotate.computeMigrationStatus(valid, d).status, 'ready');
		assert.deepEqual(Array.from(annotate.computeMigrationStatus(invalid, d).issues[0].invalidValues), [
			'GenericOnly',
		]);
	});

	test('dependent picklists use the destination controller value', () => {
		const d = describe_([
			field('Country__c', { type: 'picklist', picklistValues: [pv('USA'), pv('Canada')] }),
			field('State__c', {
				type: 'picklist',
				controllerName: 'Country__c',
				controllerValues: { USA: 0, Canada: 1 },
				picklistValues: [
					{ value: 'Arizona', label: 'Arizona', validFor: [0] },
					{ value: 'Ontario', label: 'Ontario', validFor: [1] },
				],
			}),
		]);
		const invalid = annotate.computeMigrationStatus({ values: { Country__c: 'Canada', State__c: 'Arizona' } }, d);
		assert.equal(invalid.status, 'warning');
		assert.equal(invalid.issues[0].field, 'State__c');
	});

	test('an unavailable destination record type does not borrow generic picklist options', () => {
		const d = {
			defaultRecordTypeId: '012UNAVAILABLE',
			recordTypes: [],
			fields: [
				field('Stage__c', {
					type: 'picklist',
					picklistValues: [pv('GenericOnly')],
					picklistValuesByRecordType: { '012AVAILABLE': [pv('AvailableOnly')] },
				}),
			],
		};
		const result = annotate.computeMigrationStatus(
			{ _migrateRecordTypeId: '012UNAVAILABLE', values: { Stage__c: 'Legacy' } },
			d,
		);
		assert.equal(result.status, 'ready');
	});

	test('unrestricted destination picklists preserve custom source values', () => {
		const d = describe_([
			field('Flexible__c', {
				type: 'picklist',
				restrictedPicklist: false,
				picklistValues: [pv('Suggested')],
			}),
		]);
		const rec = { values: { Flexible__c: 'Customer-defined value' } };
		const annotation = annotate.computeMigrationStatus(rec, d);

		assert.equal(annotation.status, 'ready');
		assert.equal(annotate.prepareMigrationValues(rec, annotation).Flexible__c, 'Customer-defined value');
	});
...
describe('prepareMigrationValues: destination-safe upload payload', () => {
	test('omits blank source values when updating an existing destination record', () => {
		const record = {
			_migrateMatchedId: '001TARGET',
			values: { Name: '', Phone: null, Description: '   ', Employees: 0, Active__c: false },
		};
		assert.equal(
			JSON.stringify(annotate.prepareMigrationValues(record, { issues: [] })),
			JSON.stringify({ Employees: 0, Active__c: false }),
		);
		assert.equal(record.values.Name, '');
	});

	test('preserves blank values when creating a destination record', () => {
		const record = { values: { Name: '', Description: null } };
		assert.equal(
			JSON.stringify(annotate.prepareMigrationValues(record, { issues: [] })),
			JSON.stringify({ Name: '', Description: null }),
		);
	});
...
describe('applyMigrationPlan: normal canvas materialization', () => {
	test('merges an update over its destination baseline and removes migration metadata', () => {
		const record = {
			id: 1,
			objectName: 'Account',
			values: { Name: '', Phone: '555-0100' },
			_migrateMatchedId: '001TARGET',
			_migrateMatchResolution: 'existing',
			_wasLoadedFromId: '001SOURCE',
		};
		const result = annotate.applyMigrationPlan(
			[record],
			[],
			{ 1: { issues: [] } },
			{
				'Account::001TARGET': { Id: '001TARGET', Name: 'Destination account', Rating: 'Warm' },
			},
		);
		...
		assert.equal(
			...
			JSON.stringify({ Id: '001TARGET', Name: 'Destination account', Rating: 'Warm' }),
		);
		assert.equal(
			JSON.stringify(record.values),
			JSON.stringify({ Id: '001TARGET', Name: 'Destination account', Rating: 'Warm', Phone: '555-0100' }),
		);
		assert.equal(record._migrateMatchedId, undefined);
		assert.equal(record._migrateMatchResolution, undefined);
		assert.equal(record._wasLoadedFromId, undefined);
	});

	test('keeps creates as drafts and rewrites associations to destination ids', () => {
		const account = {
			id: 1,
			objectName: 'Account',
			values: { Name: '' },
		...
		const contact = {
			id: 2,
			objectName: 'Contact',
			values: { LastName: 'Person', AccountId: '001SOURCE' },
		};
		annotate.applyMigrationPlan(
			[account, contact],
			[{ fromId: 2, toId: 1, fieldName: 'AccountId' }],
			{ 1: { issues: [] }, 2: { issues: [] } },
			{ 'Account::001TARGET': { Id: '001TARGET', Name: 'Destination account' } },
		);

		assert.equal(contact.loadedFromId, undefined);
		assert.equal(contact.values.AccountId, '001TARGET');
		assert.equal(contact.values.LastName, 'Person');
	});

	test('keeps relationships between new records as upload instructions, not stale source ids', () => {
		const account = { id: 1, objectName: 'Account', values: { Name: 'New account' } };
		const contact = {
			id: 2,
			objectName: 'Contact',
			values: { LastName: 'Person', AccountId: '001SOURCE' },
		...
		assert.equal(contact.values.AccountId, undefined);
	});

	test('does not mutate any record when a selected destination baseline is missing', () => {
		const first = { id: 1, objectName: 'Account', values: { Name: 'Draft' } };
		const second = {
			id: 2,
			objectName: 'Contact',
			values: { LastName: 'Source' },
			_migrateMatchedId: '003TARGET',
		};
		const before = JSON.stringify([first, second]);
		assert.throws(
			() => annotate.applyMigrationPlan([first, second], [], {}, {}),
			/Could not load the selected destination record/,
		);
