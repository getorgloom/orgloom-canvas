...
describe('resolveFieldValue: cross-object via savedRecords', () => {
	...
	test('a polymorphic relationship resolves the saved record matching its Salesforce ID', () => {
		const polymorphicOpts = {
			currentFields: [{ name: 'WhoId', relationshipName: 'Who', referenceTo: ['Contact', 'Lead'] }],
			savedRecords: {
				Contact: { Id: '003000000000001AAA', Name: 'Ada Contact' },
				Lead: { Id: '00Q000000000001AAA', Name: 'Grace Lead' },
			},
			describeCache: {
		...
		assert.equal(resolveFieldValue('Who.Name', { WhoId: '00Q000000000001AAA' }, polymorphicOpts), 'Grace Lead');
		assert.equal(resolveFieldValue('Who.Name', {}, polymorphicOpts), UNRESOLVED_FIELD);
	});
});
