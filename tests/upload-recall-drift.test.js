...
describe('classifyBatchDrift', () => {
	test('includes the Salesforce object label and current record name for recall review', async () => {
		const baseline = new Date(UPLOAD_TIME + 30 * 1000).toISOString();
		const inserted = [{ tempId: 't1', sfId: '001abc', objectName: 'Account', uploadLastModifiedDate: baseline }];
		const conn = makeFakeConn({
			'001abc': {
				id: '001abc',
				Name: 'Acme Holdings',
				LastModifiedById: UPLOADER_SF_ID,
				LastModifiedDate: baseline,
				IsDeleted: false,
			},
		});
		...
		assert.equal(result.clean[0].objectLabel, 'Customer Account');
		assert.equal(result.clean[0].recordName, 'Acme Holdings');
	});

	test('falls back to stored identity when the display-name query is inaccessible', async () => {
		const baseline = new Date(UPLOAD_TIME + 30 * 1000).toISOString();
		const queries = [];
		const conn = {
			version: '60.0',
			...
			batch: makeBatchRow([
				{
					...
					sfId: '001abc',
					objectName: 'Account',
					recordName: 'Stored Acme',
					uploadLastModifiedDate: baseline,
				},
			]),
			describeObject: async () => ({ label: 'Account', fields: [{ name: 'Name', nameField: true }] }),
		});
		assert.equal(queries.length, 2);
		assert.equal(result.unverified.length, 0);
		assert.equal(result.clean[0].recordName, 'Stored Acme');
	});
	...
	test('record whose current modification timestamp matches the upload baseline → clean', async () => {
		const baseline = new Date(UPLOAD_TIME + 30 * 1000).toISOString();
		const inserted = [{ tempId: 't1', sfId: '001abc', objectName: 'Account', uploadLastModifiedDate: baseline }];
		const conn = makeFakeConn({
			'001abc': {
	...
	test('record with a later modification timestamp → drifted', async () => {
		const inserted = [
			{
				tempId: 't1',
				sfId: '001abc',
				objectName: 'Account',
				uploadLastModifiedDate: new Date(UPLOAD_TIME).toISOString(),
			},
		];
		const conn = makeFakeConn({
			'001abc': {
		...
		assert.equal(r.drifted[0].driftReason, 'modified_since_upload');
		assert.equal(r.drifted[0].lastModifiedById, OTHER_SF_ID);
	});

	test('record modified by the uploader after upload is still drifted', async () => {
		const inserted = [
			{
				tempId: 't1',
				sfId: '001abc',
		...
		const conn = makeFakeConn({
			'001abc': {
				id: '001abc',
				LastModifiedById: UPLOADER_SF_ID,
				LastModifiedDate: new Date(UPLOAD_TIME + 5 * 60 * 60 * 1000).toISOString(),
				IsDeleted: false,
			},
		});
		const r = await classifyBatchDrift({
	...
	test('legacy create row without an exact upload baseline is conservatively drifted', async () => {
		...
		const r = await classifyBatchDrift({ conn, batch: makeBatchRow(inserted) });
		assert.equal(r.clean.length, 0);
		assert.equal(r.drifted.length, 1);
		assert.equal(r.drifted[0].driftReason, 'missing_upload_baseline');
	});
		...
		const inserted = [
			{
				...
				sfId: '001clean',
				objectName: 'Account',
				uploadLastModifiedDate: new Date(UPLOAD_TIME + 10 * 1000).toISOString(),
			},
			{
				tempId: 't2',
				sfId: '001drift',
			...
			{ tempId: 't3', sfId: '001gone', objectName: 'Account' },
			{
				...
				sfId: '003contact',
				objectName: 'Contact',
				uploadLastModifiedDate: new Date(UPLOAD_TIME + 20 * 1000).toISOString(),
			},
		];
		const conn = makeFakeConn({
			'001clean': {
	...
	test('15-char vs 18-char SF id comparison is case-tolerant', async () => {
		...
		const baseline = new Date(UPLOAD_TIME + 1000).toISOString();
		const inserted = [{ tempId: 't1', sfId: '001abc', objectName: 'Account', uploadLastModifiedDate: baseline }];
		const conn = makeFakeConn({
			'001abc': {
				id: '001abc',
	...
	test('elapsed time does not matter when the exact timestamp is unchanged', async () => {
		const baseline = new Date(UPLOAD_TIME + 10 * 60 * 1000).toISOString();
		const inserted = [{ tempId: 't1', sfId: '001abc', objectName: 'Account', uploadLastModifiedDate: baseline }];
		const conn = makeFakeConn({
			'001abc': {
				id: '001abc',
		...
		const result = await classifyBatchDrift({ conn, batch: makeBatchRow(inserted) });
