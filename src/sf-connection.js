...
export async function ensureSalesforceUserTimeZone(req, activeSf) {
	const session = req && req.session;
	const cached = _validTimeZone(session && session.sfAuth && session.sfAuth.timeZone);
	const userId = activeSf && activeSf.sfUserId;
	if (!activeSf || !activeSf.conn || !/^[a-zA-Z0-9]{15,18}$/.test(String(userId || ''))) {
		return cached;
	}
	try {
		const result = await activeSf.conn.query(
			"SELECT TimeZoneSidKey FROM User WHERE Id = '" + String(userId) + "' LIMIT 1",
		);
		const timeZone = _validTimeZone(result && result.records && result.records[0]?.TimeZoneSidKey);
		if (!timeZone || !session || !session.sfAuth) {
			return timeZone;
		}
		session.sfAuth.timeZone = timeZone;
		const connectionId = session.currentConnectionId;
		if (connectionId && session.sfAuthByConnection && session.sfAuthByConnection[connectionId]) {
			session.sfAuthByConnection[connectionId].timeZone = timeZone;
		}
		return timeZone;
	} catch (_) {
		return cached;
