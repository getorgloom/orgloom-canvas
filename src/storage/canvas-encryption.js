...
export function makeSfApexKekProvider(conn) {
	...
	return Object.freeze({
		...
		async wrapDataKey(dataKey) {
			...
			let response;
			try {
				response = await _apexKekRequest(conn, '/wrap', { dek: dekB64 });
			} catch (error) {
				if (!_isUnbootstrappedKekError(error)) {
					throw error;
				}
				// A reinstall can remove the protected KEK without invalidating OAuth;
				// bootstrap it once, then retry the wrapping operation.
				await _ensureSfApexKekSingleFlight(conn);
				response = await _apexKekRequest(conn, '/wrap', { dek: dekB64 });
			}
	...
	if (!res.ok) {
		...
		try {
			...
			if (parsed && (parsed.error || parsed.message)) {
				responseCode = parsed.error || 'kek-error';
				responseMessage = parsed.message || '';
				detail = responseCode + ': ' + responseMessage;
			}
		} catch (_) {
			/* response wasn't JSON */
		}
		const error = new Error('Apex KEK call failed (POST /orgloom/kek' + suffix + '): ' + detail);
...
function _isUnbootstrappedKekError(error) {
	return (
		error &&
		error.status === 400 &&
		error.responseCode === 'kek-error' &&
		typeof error.responseMessage === 'string' &&
		error.responseMessage.startsWith('KEK not bootstrapped')
	);
}

const _kekEnsureFlights = new Map();

async function _ensureSfApexKekSingleFlight(conn) {
	const key = conn.instanceUrl.replace(/\/+$/, '').toLowerCase();
	let flight = _kekEnsureFlights.get(key);
	if (!flight) {
		flight = _apexKekRequest(conn, '/ensure', {}).finally(() => {
			_kekEnsureFlights.delete(key);
		});
		_kekEnsureFlights.set(key, flight);
	}
...
export async function ensureSfApexKek(conn) {
	...
	return _ensureSfApexKekSingleFlight(conn);
}
