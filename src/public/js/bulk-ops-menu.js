	...
	window.OrgLoom.bulkOpsMenu = {
		mount: function mount(deps) {
			const required = [
				...
				'openBulkRefreshFlow',
				'beginMigration',
				'spawnPendingRecord', 'triggerTemplateFileInput',
				'getGraph', 'getCyInstance',
				'getCanvasSpaceHeld', 'setCanvasSpaceHeld',
				'getCanvasZHeld', 'setCanvasZHeld',
				'_isOnPaidPlan', 'isTeamAdmin',
			];
			if (!deps) {
...
cleanup();
			...
			function showBulkOperationsMenu(triggerEl) {
				...
				const draftHint = scopedDraftCount === 0
					? (scopeToSelected
				...
				if (canRunScripts) {
					...
					scriptItem = '<button type="button" data-bulk-op="script-not-granted" title="' + offTitle + '" style="display:flex;align-items:center;gap:0.4em;justify-content:space-between">' +
						'<span>Run script</span>' +
...
continue;
 ...
 dupesEnabled = true; break; 
				...
				if (_canBrowseForRefresh) {
					const _allLoaded = canvasState.bulkRecords.filter((r) =>
						r && !r.isTypeNode && !r.isPending && !r.pendingDelete && r.loadedFromId,
					);
					const _selectedLoaded = canvasState.bulkSelectedIds && canvasState.bulkSelectedIds.size > 0
...
openRecordDiffModal(pair[0], pair[1]);
							...
							if (isTeamAdmin()) {
								...
								window.olToast('Run script isn\'t enabled for your account. Ask a workspace admin to grant you access.', 'info');
							}
						}
					});
				});
...
return showBulkHelpPopover(hint);
			...
			function _openAutoFillModal() {
				...
				const allSelected = allRealRecords.filter((r) =>
...
total++;
				...
				function renderCountLine(mode, scope) {
					...
					if (records.length === 0) {
						const altHint = scope === 'drafts' ? 'No drafts on the canvas.'
							: scope === 'existing' ? 'No loaded Salesforce records on the canvas.'
							: 'No records selected.';
					...
					if (mode === 'clear') {
						return '<strong>' + count + '</strong> field value' + (count === 1 ? '' : 's') +
						...
						'<div class="modal-content">' +
							'<div class="af-scope">' +
								...
								'<div class="af-scope-options">' +
									'<label class="af-scope-opt' + (initialScope === 'drafts' ? ' af-scope-opt--active' : '') + (scopeDraftCount === 0 ? ' af-scope-opt--disabled' : '') + '">' +
										'<input type="radio" name="af-scope" value="drafts"' + (initialScope === 'drafts' ? ' checked' : '') + (scopeDraftCount === 0 ? ' disabled' : '') + '>' +
										'<span>All drafts</span>' +
										'<span class="tag">' + scopeDraftCount + '</span>' +
									'</label>' +
									'<label class="af-scope-opt' + (initialScope === 'existing' ? ' af-scope-opt--active' : '') + (scopeExistingCount === 0 ? ' af-scope-opt--disabled' : '') + '">' +
										'<input type="radio" name="af-scope" value="existing"' + (initialScope === 'existing' ? ' checked' : '') + (scopeExistingCount === 0 ? ' disabled' : '') + '>' +
										'<span>All existing</span>' +
										'<span class="tag">' + scopeExistingCount + '</span>' +
									'</label>' +
									'<label class="af-scope-opt' + (initialScope === 'selected' ? ' af-scope-opt--active' : '') + (scopeSelCount === 0 ? ' af-scope-opt--disabled' : '') + '">' +
										'<input type="radio" name="af-scope" value="selected"' + (initialScope === 'selected' ? ' checked' : '') + (scopeSelCount === 0 ? ' disabled' : '') + '>' +
										'<span>Selected</span>' +
										'<span class="tag">' + scopeSelCount + '</span>' +
									'</label>' +
								'</div>' +
							'</div>' +
