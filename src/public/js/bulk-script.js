					...
					'<div class="modal-content">' +
						...
						'<details class="bs-cheatsheet">' +
							...
							'<div class="bs-cheatsheet-body">' +
								...
								'<ul class="bs-vars">' +
									'<li><code>records</code> &mdash; every card on the canvas. Each has <code>id</code>, <code>objectName</code>, <code>label</code>, <code>values</code>, and (for loaded SF rows) <code>loadedFromId</code>. Edit a field with <code>r.values.FieldName = ...</code>.</li>' +
									'<li><code>r.loadedFromId</code> &mdash; the Salesforce Id of the row a card was pulled from. Set on cards loaded via Browse / SOQL / record-browse; empty on drafts you created on the canvas. At Upload time, cards <em>with</em> <code>loadedFromId</code> become UPDATEs and cards <em>without</em> become INSERTs &mdash; so this is the field to check when you want to touch only one or the other:' +
								...
								'<details class="bs-nested">' +
									...
									'<ul>' +
										...
										'<li>SOQL, DML, <code>@future</code> &mdash; not available here. Use Upload to push changes to Salesforce.</li>' +
									'</ul>' +
								'</details>' +
								'<details class="bs-nested">' +
