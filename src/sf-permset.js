...
const ORGLOOM_ACCESS_VERIFICATION = 'named-permission-set-assignment-v1';
...
function _isApiDisabledError(error) {
	const nested = Array.isArray(error?.data) ? error.data : [error?.data, error?.response?.data];
	const details = [
		error?.message,
		error?.errorCode,
		error?.code,
		error?.name,
		...nested.flatMap((entry) => [entry?.message, entry?.errorCode, entry?.code]),
	]
		.filter(Boolean)
		.join(' ');
...
function _isPackagedAccessEndpointMissing(error) {
	const codes = [error?.errorCode, error?.code, error?.data?.errorCode, error?.data?.error].filter(Boolean).join(' ');
	const messages = [error?.message, error?.data?.message].filter(Boolean).join(' ');
	return (
		_errorStatus(error) === 404 ||
		/\bNOT_FOUND\b/i.test(codes) ||
		/\bunknown-path\b/i.test(codes) ||
		/Unknown KEK operation:.*\/access/i.test(messages)
	);
}
...
async function _checkPackagedAccessPermission(conn, timeoutMs) {
	...
	try {
		...
		// Trust only the named-assignment attestation; older custom-permission results
		// can include profile grants, so fall back to direct SOQL or fail closed.
		if (response.verification !== ORGLOOM_ACCESS_VERIFICATION) {
			return null;
		}
		...
		if (_isPackagedAccessEndpointMissing(error)) {
			return null;
		}
		if (_errorStatus(error) === 403) {
			return false;
		}
		throw error;
	} finally {
