		...
		inputSchema: {
			...
			properties: {
				...
				summary: {
					type: "string",
					description:
						"Human-readable summary of what these changes accomplish",
					maxLength: 500,
				},
				changes: {
					type: "array",
					description:
						"Per-record / per-draft / per-association changes",
					items: {
						type: "object",
						properties: {
							kind: {
							...
							from: {
								...
								properties: {
									recordId: {
										type: "string",
										description:
											"Existing loaded record id (15 or 18 chars).",
									},
									tempId: {
										type: "number",
									...
									tempRef: {
										type: "string",
										description:
											"tempRef of a new-draft in this same proposal.",
									},
								},
								additionalProperties: false,
							},
		...
		inputSchema: {
			...
			properties: {
				objectName: {
					type: "string",
					description:
						'SObject API name (e.g. "Task", "Account", "Contact"). Case-sensitive.',
				},
				fields: {
					type: "array",
					description:
		...
		inputSchema: {
			...
			properties: {
				proposalId: {
					type: "string",
					description:
						"The proposal id returned from your earlier propose_record_changes call.",
				},
			},
			required: ["proposalId"],
			additionalProperties: false,
		...
		inputSchema: {
			...
			properties: {
				canvasId: {
					type: "string",
					description:
						"Canvas id where the question should surface. Use the id from list_canvases.",
				},
				question: {
					type: "string",
					description:
		...
		inputSchema: {
			...
			properties: {
				clarificationId: {
					type: "string",
					description:
						"The clarification id returned from request_clarification.",
				},
			},
...
async function _toolProposeRecordChanges(ctx, args) {
	...
	try {
		...
		throw _appError(
			ERR_INTERNAL,
			"Relay error during proposal pre-validation: " +
				(code || "unknown"),
		);
	}
	const payload = (liveRead && liveRead.payload) || {};
	const loadedRecords = Array.isArray(payload.loadedRecords)
	...
	const cachedAssociations = Array.isArray(payload.associations)
	...
	for (const c of changes) {
		...
		if (c.tempRef != null && c.tempRef !== "") {
			...
			if (!/^[a-zA-Z0-9_-]{1,64}$/.test(ref)) {
				throw _appError(
					ERR_INVALID_PARAMS,
					"change.tempRef must be 1-64 chars of [a-zA-Z0-9_-]",
				);
			}
	...
	function _normalizeAssocEndpoint(label, e) {
		if (!e || typeof e !== "object" || Array.isArray(e)) {
			throw _appError(
				ERR_INVALID_PARAMS,
				label +
					" must be an object with exactly one of recordId / tempId / tempRef",
			);
		}
		const keys = ["recordId", "tempId", "tempRef"].filter(
			(k) => e[k] != null && e[k] !== "",
		);
		if (keys.length !== 1) {
			throw _appError(
				ERR_INVALID_PARAMS,
				label +
					" must have EXACTLY ONE of recordId / tempId / tempRef (got " +
					keys.length +
					")",
			);
		}
		...
		if (key === "recordId") {
			...
			if (!/^[a-zA-Z0-9]{15,18}$/.test(id)) {
				throw _appError(
					ERR_INVALID_PARAMS,
					label + ".recordId must be a 15- or 18-char Salesforce id",
				);
			}
			if (!recordById.has(id)) {
				throw _appError(
					...
					label +
						".recordId " +
						id +
						" is not a loaded record on this canvas (per the cache)",
				);
			}
		...
		if (key === "tempId") {
			...
			if (!Number.isInteger(n)) {
				throw _appError(
					ERR_INVALID_PARAMS,
					label + ".tempId must be an integer",
				);
			}
			if (!draftByTempId.has(String(n))) {
				throw _appError(
					ERR_INVALID_PARAMS,
					label + ".tempId " + n + " is not a draft on this canvas",
				);
			}
		...
		if (!tempRefIndex.has(ref)) {
			throw _appError(
				...
				label +
					'.tempRef "' +
					ref +
					'" does not match any new-draft tempRef in this proposal',
			);
		}
	...
	for (const c of changes) {
		...
		if (c.kind === "delete-draft") {
			...
			if (!draftByTempId.has(tempIdStr)) {
				throw _appError(
					...
					"delete-draft.tempId " +
						tempIdStr +
						" is not a draft on this canvas (per the live read)",
				);
			}
			const tempIdValue = /^-?\d+$/.test(tempIdStr)
				? Number(tempIdStr)
				: tempIdStr;
		...
		if (c.kind === "delete-record") {
			...
			if (!/^[a-zA-Z0-9]{15,18}$/.test(c.recordId)) {
				throw _appError(
					ERR_INVALID_PARAMS,
					"delete-record.recordId must be a 15- or 18-char Salesforce id",
				);
			}
		...
		if (c.kind === "load-record") {
			if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(String(c.objectName || ""))) {
				throw _appError(
					ERR_INVALID_PARAMS,
					"load-record.objectName must be a valid SObject API name",
				);
			}
			if (
				!c.recordId ||
				typeof c.recordId !== "string" ||
				!/^[a-zA-Z0-9]{15,18}$/.test(c.recordId)
			) {
				throw _appError(
					ERR_INVALID_PARAMS,
					"load-record.recordId must be a 15- or 18-char Salesforce id",
				);
			}
		...
		if (c.kind === "autofill-required") {
			...
			for (const raw of tempIds) {
				...
				if (!draftByTempId.has(String(n))) {
					throw _appError(
						ERR_INVALID_PARAMS,
						"autofill-required.tempIds[" +
							n +
							"] is not a draft on this canvas",
					);
				}
		...
		if (c.kind === "new-association" || c.kind === "delete-association") {
			if (
				...
				!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(c.fieldName)
			) {
				throw _appError(
					ERR_INVALID_PARAMS,
		...
		if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(String(c.objectName || ""))) {
			throw _appError(
				ERR_INVALID_PARAMS,
				"change.objectName must be a valid SObject API name",
			);
		}
		if (
			!c.fields ||
			...
			throw _appError(
				ERR_INVALID_PARAMS,
				"change.fields must be an object",
			);
		}
		const hasRecordId = c.recordId != null && c.recordId !== "";
		const hasTempId = c.tempId != null && c.tempId !== "";
		...
		if (hasRecordId) {
			if (!/^[a-zA-Z0-9]{15,18}$/.test(String(c.recordId))) {
				throw _appError(
					ERR_INVALID_PARAMS,
					"change.recordId must be a 15- or 18-char Salesforce id",
				);
			}
			...
			for (const k of Object.keys(c.fields)) {
				oldValues[k] =
					target.values && target.values[k] != null
						? target.values[k]
						: null;
			}
			normalized.push({
	...
	const newDraftCount = normalized.filter(
		(c) => c.kind === "new-draft",
	).length;
	const newAssocCount = normalized.filter(
		(c) => c.kind === "new-association",
	).length;
	const delAssocCount = normalized.filter(
		(c) => c.kind === "delete-association",
	).length;
	const delDraftCount = normalized.filter(
		(c) => c.kind === "delete-draft",
	).length;
	const delRecordCount = normalized.filter(
		(c) => c.kind === "delete-record",
	).length;
	const autofillCount = normalized.filter(
		(c) => c.kind === "autofill-required",
	).length;
	const loadRecordCount = normalized.filter(
		(c) => c.kind === "load-record",
	).length;
	const parts = [];
	if (newDraftCount) {
...
async function _toolDescribeObject(ctx, args) {
	...
	try {
		...
		const code = err && err.message;
		if (
			code === "no-live-browser-for-canvas" ||
			code === "relay-connection-dropped"
		) {
			throw _appError(
				ERR_NOT_FOUND,
...
async function _toolWithdrawProposal(ctx, args) {
	...
	if (
		proposal.proposingTokenId &&
		ctx.mcpToken &&
		ctx.mcpToken.id !== proposal.proposingTokenId
	) {
		throw _appError(
			ERR_AUTH,
	...
	return _textResult(
		JSON.stringify({
			...
			status: "withdrawn",
			message:
				"Proposal withdrawn. It will disappear from the user-facing review banner on their next poll.",
		}),
	);
}
...
async function _toolReadProposalOutcome(ctx, args) {
	...
	if (
		appliedRow &&
		(!rejectedRow ||
			(appliedRow.createdAt || 0) >= (rejectedRow.createdAt || 0))
	) {
	...
	if (terminalRow) {
		const payloadTokenId =
			terminalRow.payload && terminalRow.payload.proposingTokenId;
		if (
			payloadTokenId &&
			ctx.mcpToken &&
			ctx.mcpToken.id !== payloadTokenId
		) {
			throw _appError(
				ERR_NOT_FOUND,
				"Proposal " + proposalId + " not found.",
		...
		const response = {
			proposalId,
			canvasId:
				(terminalRow.payload && terminalRow.payload.canvasId) || null,
		...
		if (
			terminalStatus === "applied" &&
			terminalRow.payload &&
			Array.isArray(terminalRow.payload.results)
		) {
...
async function _toolGetCanvasSummary(ctx, args) {
	...
	try {
		...
		if (code === "relay-connection-dropped") {
			throw _appError(
				ERR_NOT_FOUND,
				"The user's browser disconnected before the canvas could be read.",
			);
		}
		if (code === "relay-request-timeout") {
			throw _appError(
	...
	if (!canvas) {
		throw _appError(
			ERR_NOT_FOUND,
			"Canvas " + canvasId + " did not return a payload.",
		);
	}

	const drafts = Array.isArray(canvas.drafts) ? canvas.drafts : [];
	const loaded = Array.isArray(canvas.loadedRecords)
		? canvas.loadedRecords
		: [];
	const associations = Array.isArray(canvas.associations)
		? canvas.associations
		: [];
	const byObject = {};
	for (const d of drafts) {
	...
	try {
		...
		pendingProposalCount = pending.filter(
...
async function _toolGetMyCapabilities(ctx) {
	...
	try {
		...
		workspaceCreditsDb =
			await import("orgloom-saas/database/workspace-credits");
	} catch (_e) {
		return _textResult(
			JSON.stringify({
				workspace: {
	...
	const plan =
		(await workspacesDb.getPlan(ctx.workspaceId)) || planById(null);
	...
	try {
		...
		try {
			creditsRemaining = await workspaceCreditsDb.getBalance(
				ctx.workspaceId,
			);
		} catch (_e) {}
		usage = {
			...
			uploads: {
				...
				remaining:
					uploadsCap == null
						? null
						: Math.max(0, uploadsCap - uploadsUsed),
			},
		};
	} catch (_e) {
		/* leave empty */
...
async function _toolRequestClarification(ctx, args) {
	...
	if (rawOptions != null) {
		if (!Array.isArray(rawOptions)) {
			throw _appError(
				ERR_INVALID_PARAMS,
				"options must be an array of strings when provided.",
			);
		}
		if (rawOptions.length > 6) {
			throw _appError(
				ERR_INVALID_PARAMS,
				"options is capped at 6 entries.",
			);
		}
		options = rawOptions
	...
	if (!aiClarifications || typeof aiClarifications.create !== "function") {
		throw _appError(
			ERR_INTERNAL,
			"Clarifications storage is not available in this deployment.",
		);
	}
...
async function _toolReadClarification(ctx, args) {
	...
	if (
		row.requestingTokenId &&
		ctx.mcpToken &&
		ctx.mcpToken.id !== row.requestingTokenId
	) {
		throw _appError(
			ERR_NOT_FOUND,
			"Clarification " + clarificationId + " not found.",
...
export async function mcpHandler(req, res) {
	...
	if (!body || typeof body !== "object") {
		return _sendError(
			...
			null,
			ERR_PARSE,
			"Body must be a JSON-RPC 2.0 object",
		);
	}
	if (body.jsonrpc !== "2.0") {
		return _sendError(
