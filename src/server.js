...
app.get('/', async (req, res, next) => {
	try {
		// Always revalidate the app shell so reloads pick up current script revisions.
		...
		const activeSf = await getActiveSfConnection(req);
		const sfTimeZone = await ensureSalesforceUserTimeZone(req, activeSf);
		res.render('index', {
			user: {
				id: account.id,
				username: account.email,
