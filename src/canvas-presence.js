...
function _visibilityForRef(entry, reference) {
	if (!entry.visibility || entry.role === 'owner') {
		return { visible: true, readableFields: null, encryptedFields: null };
	}
	if (!reference || typeof reference !== 'object') {
		return { visible: false, readableFields: null, encryptedFields: null };
	}
	if (reference.refKind === 'draft') {
		...
		return draft && draft.visible
			? {
					visible: true,
					readableFields: new Set(draft.readableFields || []),
					encryptedFields: new Set(draft.encryptedFields || []),
				}
			: { visible: false, readableFields: null, encryptedFields: null };
	}
	if (reference.refKind === 'loaded') {
		...
		return record
			? {
					visible: true,
					readableFields: new Set(record.readableFields || []),
					encryptedFields: new Set(record.encryptedFields || []),
				}
			: { visible: false, readableFields: null, encryptedFields: null };
	}
...
function _pickReadableFields(values, allowed, encryptedFields = null) {
	if (!values || typeof values !== 'object' || !allowed) {
		return {};
	}
	const projected = {};
	for (const [name, value] of Object.entries(values)) {
		if (allowed.has(name) && !(encryptedFields && encryptedFields.has(name))) {
...
function _redactedEncryptedFields(payload, encryptedFields) {
	if (!(encryptedFields instanceof Set) || encryptedFields.size === 0) {
		return [];
	}
	const present = new Set([
		...Object.keys((payload && payload.fields) || {}),
		...Object.keys((payload && payload.baseline) || {}),
	]);
	return Array.from(encryptedFields).filter((name) => present.has(name));
}

function _encryptedFieldsForPayload(entry, payload) {
	const visibility = entry && entry.visibility;
	if (!visibility || !payload) {
		return new Set();
	}
	if (payload.objectName) {
		return new Set(visibility.objects?.[String(payload.objectName)]?.encryptedFields || []);
	}
	if (payload.type === 'loaded-record' && payload.sfId) {
		return new Set(visibility.loadedRecords?.[_sfIdKey(payload.sfId)]?.encryptedFields || []);
	}
	const reference = payload.targetRef;
	if (!reference || typeof reference !== 'object') {
		return new Set();
	}
	if (reference.refKind === 'loaded') {
		return new Set(visibility.loadedRecords?.[_sfIdKey(reference.ref)]?.encryptedFields || []);
	}
	if (reference.refKind === 'draft') {
		return new Set(visibility.drafts?.[String(reference.ref)]?.encryptedFields || []);
	}
	if (reference.refKind === 'slot') {
		const slot = visibility.slots?.[String(reference.ref)];
		if (slot?.loadedRecordId) {
			return new Set(visibility.loadedRecords?.[_sfIdKey(slot.loadedRecordId)]?.encryptedFields || []);
		}
		if (slot?.draftId != null) {
			return new Set(visibility.drafts?.[String(slot.draftId)]?.encryptedFields || []);
		}
	}
	return new Set();
}

function _projectOwnerPresencePayload(entry, payload) {
	if (!['loaded-record', 'draft-update', 'field-update'].includes(payload.type)) {
		return payload;
	}
	const encryptedFields = _encryptedFieldsForPayload(entry, payload);
	if (encryptedFields.size === 0) {
		return payload;
	}
	const projected = {
		...payload,
		fields: _pickReadableFields(payload.fields, new Set(Object.keys(payload.fields || {})), encryptedFields),
	};
	if (payload.baseline) {
		projected.baseline = _pickReadableFields(
			payload.baseline,
			new Set(Object.keys(payload.baseline)),
			encryptedFields,
		);
	}
	if (payload.type === 'loaded-record') {
		const redactedFields = _redactedEncryptedFields(payload, encryptedFields);
		if (redactedFields.length > 0) {
			projected.redactedFields = redactedFields;
		}
	}
	return projected;
}
...
function _projectPresencePayload(entry, payload) {
	...
	if (entry.role === 'owner') {
		return _projectOwnerPresencePayload(entry, payload);
	}
	if (payload.type === 'loaded-record') {
		...
		const redactedFields = _redactedEncryptedFields(payload, visibility.encryptedFields);
		return {
			...payload,
			fields: _pickReadableFields(payload.fields, visibility.readableFields, visibility.encryptedFields),
			...(payload.baseline
				? {
						baseline: _pickReadableFields(
							payload.baseline,
							visibility.readableFields,
							visibility.encryptedFields,
						),
					}
				: {}),
	...
	if (payload.type === 'field-update') {
		...
		const fields = visibility.readableFields
			? _pickReadableFields(payload.fields, visibility.readableFields, visibility.encryptedFields)
			: payload.fields;
