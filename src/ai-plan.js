...
// Validates and normalizes model output against the caller's allowed Salesforce schema.
...
export function buildAiDescribeSummary(describes, fkFieldsByObject) {
	return describes
		.map(({ name, describe }) => {
			const allowFkSet =
				fkFieldsByObject && Array.isArray(fkFieldsByObject[name]) ? new Set(fkFieldsByObject[name]) : null;
...
export function buildAiSystemPrompt(summary) {
	return (
		...
		'8. Keep the plan tight. Generate at most ' +
		AI_MAX_RECORDS +
		' records, and prefer fewer realistic records over larger counts.\n' +
				...
				items: {
					...
					properties: {
						...
						values: {
							type: 'object',
							description:
								"Field API name → value. Only set fields from the object's listed schema. Do not set reference fields here; use associations.",
...
export function validateAiPlan(plan, describes) {
	...
	if (rawRecords.length > AI_MAX_RECORDS) {
		warnings.push(
			'Plan contained ' + rawRecords.length + ' records; only the first ' + AI_MAX_RECORDS + ' were kept.',
		);
	}
	rawRecords.slice(0, AI_MAX_RECORDS).forEach((r, idx) => {
		...
		if (!describe) {
			warnings.push(
				'Record tempId=' + r.tempId + ' targets unknown object "' + r.objectName + '" and was dropped.',
			);
			return;
		}
		...
		Object.entries(r.values || {}).forEach(([fname, fval]) => {
			...
			if (!f) {
				warnings.push(
					'tempId=' + r.tempId + ' (' + r.objectName + '): unknown field "' + fname + '" was dropped.',
				);
				return;
			}
			if (!f.createable) {
				warnings.push(
					'tempId=' + r.tempId + ' (' + r.objectName + '): non-createable field "' + fname + '" was dropped.',
				);
				return;
			}
			if (f.type === 'reference') {
				warnings.push(
					'tempId=' +
						...
						'): reference field "' +
						fname +
						'" must come from an association, so it was dropped.',
				);
				return;
			}
			if (f.type === 'picklist') {
				...
				if (!allowed.has(fval)) {
					warnings.push(
						'tempId=' +
							...
							'" value "' +
							fval +
							'" is not allowed and was dropped.',
					);
					return;
				}
			}
				...
				if (valid.length === 0) {
					warnings.push(
						'tempId=' +
							...
							' (' +
							r.objectName +
							'): no valid multipicklist values in "' +
							fname +
	...
	(plan.associations || []).forEach((a) => {
		...
		if (!from || !to) {
			warnings.push(
				'Association references missing record (' + a.fromTempId + ' → ' + a.toTempId + '); dropped.',
			);
			return;
		}
		...
		if (!field) {
			warnings.push('Association field "' + a.fieldName + '" not on ' + from.objectName + '; dropped.');
			return;
		}
		if (field.type !== 'reference') {
			warnings.push(
				'Association field "' + a.fieldName + '" on ' + from.objectName + ' is not a reference; dropped.',
			);
			return;
		}
		if (Array.isArray(field.referenceTo) && !field.referenceTo.includes(to.objectName)) {
			warnings.push(
				'Association target type mismatch on ' +
					from.objectName +
					'.' +
					a.fieldName +
					' (expected ' +
		...
		if (!field.createable) {
			warnings.push(
				'Association field "' + a.fieldName + '" on ' + from.objectName + ' is not createable; dropped.',
			);
			return;
		}
