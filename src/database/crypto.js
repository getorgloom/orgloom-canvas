...
const _key = process.env.ENCRYPTION_KEY ? Buffer.from(process.env.ENCRYPTION_KEY, 'hex') : null;
...
export function encrypt(plaintext) {
	if (!_ready) {
		throw new Error('Encryption not configured: set ENCRYPTION_KEY (64 hex chars) on the server.');
	}
...
export function decrypt(encoded) {
	...
	const decipher = crypto.createDecipheriv('aes-256-gcm', _key, iv, { authTagLength: 16 });
