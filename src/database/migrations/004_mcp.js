...
export async function up(db) {
	...
	const hasWorkspaces = await migrationTableExists(db, 'workspaces');
	if (hasWorkspaces) {
		await db.schema
	...
	if (await migrationTableExists(db, 'workspace_settings')) {
		await db.schema
			...
			.addColumn('ai_on_canvas_data_enabled', 'integer', (col) => col.notNull().defaultTo(0))
			.execute();
		await db.schema
			.alterTable('workspace_settings')
			.addColumn('mcp_required_review', 'integer', (col) => col.notNull().defaultTo(1))
			.execute();
	}
...
export async function down(db) {
	await db.schema.alterTable('audit_log').dropColumn('mcp_token_id').execute();
	await db.schema.alterTable('audit_log').dropColumn('actor_kind').execute();
	if (await migrationTableExists(db, 'workspace_settings')) {
		await db.schema.alterTable('workspace_settings').dropColumn('mcp_required_review').execute();
		await db.schema.alterTable('workspace_settings').dropColumn('ai_on_canvas_data_enabled').execute();
	}
