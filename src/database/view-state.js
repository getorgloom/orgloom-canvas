...
export async function get(accountId) {
	...
	const db = ext.getDb();
	return db.selectFrom('account_view_state').selectAll().where('account_id', '=', accountId).executeTakeFirst();
}
...
export async function set(accountId, patch) {
	...
	if (existing) {
		...
		if (Object.prototype.hasOwnProperty.call(patch, 'currentConnectionId')) {
			merged.current_connection_id = patch.currentConnectionId || null;
		}
		await db.updateTable('account_view_state').set(merged).where('account_id', '=', accountId).execute();
	} else {
		await db
