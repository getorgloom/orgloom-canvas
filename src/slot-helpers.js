...
function _objectAccess(accessByObject, objectName) {
	...
	const encryptedFields =
		raw.encryptedFields instanceof Set
			? raw.encryptedFields
			: new Set(Array.isArray(raw.encryptedFields) ? raw.encryptedFields : []);
	return {
...
function _shareableValueFields(objectAccess) {
	const readable = objectAccess && objectAccess.readableFields;
	if (!(readable instanceof Set)) {
		return readable;
	}
	const fields = objectAccess.fields instanceof Map ? objectAccess.fields : new Map();
	const encrypted = objectAccess.encryptedFields instanceof Set ? objectAccess.encryptedFields : new Set();
	return new Set(
		Array.from(readable).filter((name) => {
			const field = fields.get(name);
			return !encrypted.has(name) && (!field || String(field.type || '').toLowerCase() !== 'encryptedstring');
		}),
	);
}

function _encryptedFieldsByObject(payload, accessByObject) {
	const result = new Map();
	const access = accessByObject instanceof Map ? accessByObject : new Map();
	for (const [objectName, objectAccess] of access) {
		const encrypted = objectAccess && objectAccess.encryptedFields;
		result.set(
			objectName,
			new Set(encrypted instanceof Set ? encrypted : Array.isArray(encrypted) ? encrypted : []),
		);
	}
	for (const object of Array.isArray(payload?.schema?.objects) ? payload.schema.objects : []) {
		if (!object || !object.name) {
			continue;
		}
		if (!result.has(object.name)) {
			result.set(object.name, new Set());
		}
		for (const field of Array.isArray(object.draftFields) ? object.draftFields : []) {
			if (field && field.name && String(field.type || '').toLowerCase() === 'encryptedstring') {
				result.get(object.name).add(field.name);
			}
		}
	}
	return result;
...
export function stripEncryptedCanvasValues(payload, accessByObject) {
	const source = payload && typeof payload === 'object' ? payload : {};
	const encryptedByObject = _encryptedFieldsByObject(source, accessByObject);
	const cleanRecord = (record, valueKeys) => {
		if (!record || typeof record !== 'object') {
			return record;
		}
		const encrypted = encryptedByObject.get(record.objectName) || new Set();
		const safe = Object.assign({}, record);
		const intents = new Set(
			(Array.isArray(record.encryptedFieldIntents) ? record.encryptedFieldIntents : []).filter((fieldName) =>
				encrypted.has(fieldName),
			),
		);
		for (const key of valueKeys) {
			...
			const values = Object.assign({}, record[key]);
			...
			if (Object.keys(values).length > 0 || key === 'values') {
				safe[key] = values;
			} else {
				delete safe[key];
			}
		...
		if (intents.size > 0) {
			safe.encryptedFieldIntents = Array.from(intents).sort();
		} else {
			delete safe.encryptedFieldIntents;
		}
		if (safe.slot && (safe.slot.kind || 'whole-record') === 'fields') {
			safe.slot = Object.assign({}, safe.slot, {
				fields: (Array.isArray(safe.slot.fields) ? safe.slot.fields : []).filter(
					(fieldName) => !encrypted.has(fieldName),
				),
			});
		}
		return safe;
	...
	return Object.assign({}, source, {
		loadedRecords: (Array.isArray(source.loadedRecords) ? source.loadedRecords : []).map((record) =>
			cleanRecord(record, ['values', 'loadedValues', 'changes']),
		),
		drafts: (Array.isArray(source.drafts) ? source.drafts : []).map((record) =>
			cleanRecord(record, ['values', 'loadedValues', 'changes']),
		),
	});
}
...
export function projectSharedCanvasPayload(payload, accessByObject) {
	...
	for (const record of Array.isArray(source.loadedRecords) ? source.loadedRecords : []) {
		...
		const readable = objectAccess.readableFields;
		const allowed = _shareableValueFields(objectAccess);
	...
	for (const record of Array.isArray(source.drafts) ? source.drafts : []) {
		...
		const values = _pickAllowedValues(record.values, _shareableValueFields(objectAccess));
		delete projected.values;
		if (values) {
			projected.values = values;
		}
		if (projected.slot) {
			projected.slot = _projectSlot(projected.slot, _shareableValueFields(objectAccess));
		}
...
function _editorFieldWritable(access, fieldName, kind) {
	...
	const encrypted = access && access.encryptedFields instanceof Set && access.encryptedFields.has(fieldName);
	return !!(
		field &&
		!encrypted &&
		String(field.type || '').toLowerCase() !== 'encryptedstring' &&
		(kind === 'loaded' ? field.updateable : field.createable)
	);
}
