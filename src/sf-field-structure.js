...
export function isExternalKeyReference(field) {
	return !!(
		field &&
...
export function isRequiredOnCreate(field) {
	return isCreateInputField(field) && field.nillable === false && !field.defaultedOnCreate;
}

export function isCreateInputField(field) {
	return !!field && !!field.createable && !field.calculated && !field.autoNumber && !isCompoundContainer(field);
}
...
export function isPolymorphicReference(field) {
	return !!field && field.type === 'reference' && Array.isArray(field.referenceTo) && field.referenceTo.length > 1;
}
